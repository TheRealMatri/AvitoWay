...
greeting_text = load_txt('greeting.txt') or "Добро пожаловать в Avito AI Чат!"
...
class UserState:
    def __init__(self):
        self.phase = "init"  # init -> product_info -> delivery -> order -> complete
...
def is_available(availability_str):
    """Check product availability based on string representations"""
...
def normalize_model_name(model_name):
    """Standardize iPhone model names for matching"""
    if not model_name:
        return ""
...
def normalize_storage(storage):
    """Standardize storage size formatting"""
    if not storage:
        return ""
...
def normalize_color(color):
    """Standardize color names for matching"""
    if not color:
        return ""
...
def find_matching_products(products, model=None, storage=None, color=None):
    """Find products matching user criteria"""
    results = []
...
def get_available_colors(products, model, storage):
    """Get available colors for model/storage combination"""
    return list(set(
...
def find_similar_models(user_input, available_models):
    """Suggest similar models when exact match not found"""
    ...
    # Third pass: variant keyword matches
    if not suggestions:
        for model in available_models:
            norm_model = normalize_model_name(model)
            match = True
    ...
    # Fourth pass: similarity matching
    if not suggestions:
        similarity_scores = []
        for model in available_models:
            norm_model = normalize_model_name(model)
    ...
    # Fifth pass: difflib as fallback
    if not suggestions:
...
def format_order_summary(order_data):
    """Format order details for user confirmation"""
    summary = "📝 Ваш заказ:\n"
...
# Avito Integration Functions
def get_avito_access_token():
    """Get OAuth token from Avito API"""
    ...
    if response.status_code == 200:
        ...
        logger.error(f"Failed to get Avito token: {response.status_code} - {response.text}")
        return None

def send_avito_message(chat_id, content):
    ...
    url = f"https://api.avito.ru/messenger/v2/accounts/{AVITO_CLIENT_ID}/chats/{chat_id}/messages"
    headers = {
    ...
    try:
        ...
        logger.error(f"Error sending to Avito: {str(e)}")
        return False

def verify_avito_signature(payload, signature):
    """Verify webhook signature from Avito"""
    if not AVITO_WEBHOOK_SECRET:
        logger.warning("Webhook verification skipped - no secret configured")
        return True

    digest = hmac.new(
        AVITO_WEBHOOK_SECRET.encode(),
        payload.encode(),
        hashlib.sha256
    ).hexdigest()
...
# Uptime endpoint to prevent shutdown
@app.route('/ping')
def ping():
    return "pong", 200

# Background thread to keep app alive
def keep_alive():
    """Prevent Render from sleeping by pinging app"""
    while True:
        try:
...
# Main webhook endpoint for Avito
...
def avito_webhook():
    try:
        ...
        if not verify_avito_signature(body, signature):
        ...
        if event == 'message':
            ...
            user_input = message_data.get('content', {}).get('value', '').strip()
            ...
            # Process the message
            response = process_avito_message(chat_id, user_input)

            # Send response via Avito API
            if isinstance(response, str):
                send_avito_message(chat_id, response)
            elif isinstance(response, dict) and "reply" in response:
                send_avito_message(chat_id, response["reply"])
...
def process_avito_message(chat_id, user_input):
    """Core chat message processing logic"""
    try:
        ...
        if chat_id not in user_states:
            ...
            logger.info(f"New session for chat: {chat_id}")
        ...
        if user_state.phase == "init":
            response = handle_product_inquiry(user_input, user_state, chat_id)
        elif user_state.phase == "product_info":
            response = handle_product_info_response(user_input, user_state, chat_id)
            ...
            response = handle_order_form_step(user_input, user_state, chat_id)
        elif user_state.phase == "complete":
            response = handle_complete_phase(user_input, user_state, chat_id)
        else:
        ...
        if user_state.reset_context:
            if chat_history and chat_history[-1]["role"] == "assistant":
                chat_histories[chat_id] = [chat_history[-1]]
            user_state.reset_context = False
...
def handle_complete_phase(user_input, user_state, chat_id):
    """Handle post-order completion interactions"""
    if any(word in user_input.lower() for word in ["новый", "еще", "другой", "ещё"]):
        ...
        user_state.phase = "init"
        return handle_product_inquiry(user_input, user_state, chat_id)

def extract_models_from_input(user_input):
    """Extract iPhone models from user input"""
    models = []
    patterns = [
...
def handle_product_inquiry(user_input, user_state, chat_id):
    """Handle initial product inquiries"""
    ...
    products = get_available_products()
    prev_messages = chat_histories.get(chat_id, [])
...
def handle_product_info_response(user_input, user_state, chat_id):
    """Handle detailed product information requests"""
...
def handle_order_form_step(user_input, user_state, chat_id):
    """Process order form steps"""
    products = get_available_products()
...
def match_delivery_option(text):
    """Match delivery method from user input"""
    text = text.lower()
    ...
    elif "авито" in text or "avito" in text:
        return "Авито Доставка"
    return None

def get_office_status():
    """Get current office status from Google Sheet"""
    try:
